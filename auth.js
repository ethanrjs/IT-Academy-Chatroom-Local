...
io.on('connection', socket => {
    socket.on('disconnect', () => {
        console.log(chalk.bgBlueBright.white(' [INFO] ') + ' ' + chalk.white('User disconnected'));
        io.emit('users', {
    ...
    socket.on('chatroom', message => {
        ...
        if (msg.message.length > 4000) msg.message = msg.message.substring(0, 4000);
        msg = JSON.stringify(msg);
        io.emit('chatroom', msg);
    });

    socket.on('users', msg => {
        console.log(chalk.bgBlueBright.white(' [INFO] ') + ' ' + `${msg.username} connected.`);
        let ip = socket.handshake.address;
        console.log(chalk.bgBlueBright.white(' [INFO] ') + ' ' + chalk.white('User connected from ' + ip));

        const banlist = require('./banlist.json');
        if (banlist.list.includes(ip)) {
            console.log(chalk.bgBlueBright.white(' [INFO] ') + ' ' + chalk.white('User banned from ' + ip));
