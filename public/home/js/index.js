...
document.querySelector('#chatroom-input').addEventListener('keydown', e => {
    if (e.key === 'Enter') {
        e.preventDefault();
        if (document.querySelector('#chatroom-input').value.split('```').length % 2 === 1) {
            send();
        ...
        if (!(document.querySelector('#chatroom-input').value.split('```').length % 2 === 0)) return;
        e.preventDefault();
        let start = document.querySelector('#chatroom-input').selectionStart;
        let end = document.querySelector('#chatroom-input').selectionEnd;
        document.querySelector('#chatroom-input').value =
            document.querySelector('#chatroom-input').value.substring(0, start) +
            '    ' +
            document.querySelector('#chatroom-input').value.substring(end);
        document.querySelector('#chatroom-input').selectionStart = document.querySelector(
            '#chatroom-input'
