class ChatMessage {
    ...
    static send(message = {}) {
        ...
        */
        ...
        if (message.pfp) {
            var pfp = `/profilePicture/${message.sender}?${new Date().getTime()}`;
        }

        // get HH:MM for timestamp
        let formattedTime = new Date(message.timestamp).toLocaleTimeString('en-US', {
            hour: 'numeric',
            minute: 'numeric'
        });
        ...
        try {
            ...
            let lastMessageUser = lastMessage.querySelector('.message-sender p').innerHTML;
